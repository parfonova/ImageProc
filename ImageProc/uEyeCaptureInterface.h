...
//							IuEyeImageForcv::Mat		- specific uEye interface
...
const char uEyeCaptureInterfaceVersion[] = "3.0.17";
...
interface IuEyeCapture : public IUnknown
{
	...
	*		inforcv::Mation of the available cameras.
	*/ 
...
interface IuEyeAutoFeatures : public IUnknown
{
	...
	* \brief 	Queries the actual set reference value for auto brightness control.
    ...
    /*!
	* \brief 	Upper limit of the exposure time when used to control the image brightness autocv::Matically.  
    ...
    /*!
	* \brief 	Queries the actual set upper limit of autocv::Matic controlled exposure time.
    ...
    /*!
	* \brief 	Upper limit of gain when used to control the image brightness autocv::Matically.
    ...
    /*!
	* \brief 	Queries the actual set upper limit of autocv::Matic controlled master gain amplifier. 
    ...
    /*!
	* \brief 	Controls the percentage of examined images for the autocv::Matic brightness control unit.
	...
	* \brief 	Queries the actual used area of interest in which the brightness is examined.
    ...
    /*!
	* \brief 	Specifies relative offsets between the individual color channels when used by the autocv::Matic whitebalance control unit.
    ...
    /*!
	* \brief 	Queries the actual set color channel offsets for autocv::Matic whitebalance.
    ...
    /*!
	* \brief 	Limits the range the autocv::Matic whitebalance controller unit is allowed to use when adjusting the RGB gain channels.
    ...
    /*!
	* \brief 	Queries the actual allowed gain range for the autocv::Matic whitebalance controller unit.
    ...
    /*!
	* \brief	Controls the percentage of examined images for the autocv::Matic whitebalance control unit.
     *  \param  lSpeed      The desired speed in a range of 1%(slow) to 100%(fast).
     *  \return HRESULT     0 on success, error code otherwise.
...
interface IuEyeAOI : public IUnknown
{
     ...
     * \param   prcAOI   output location containing aoi inforcv::Mation (left, top, right, bottom).
	 * \return	HRESULT	S_OK on success, error code otherwise.
     */
    ...
    /*!
     * \brief Get the current image area of interest in absolute uEye coordinates.
     * \param   pRectAOI   output location containing aoi inforcv::Mation (x, y, width, height).
	 * \return	HRESULT	S_OK on success, error code otherwise.
     */
...
interface IuEyeScaler : public IUnknown
{
	...
	* \brief	Activated in some sensors the internal image scaling
	...
	/*!	 
	* \brief	Obtained from some sensors inforcv::Mation about the internal image scaling
     *	\param 	pSensorScalerInfo		Pointer to a structure of type SENSORSCALERINFO, 
	 *									where should be written in the inforcv::Mation
...
// ============================================================================
/*! \defgroup IuEyeImageForcv::Mat uEye image forcv::Mat Interface
* 
*  Proprietary interface for controlling uEye image forcv::Mat features exposed by 
*  the capture filter. 
*  Allows a DirectShow based program to control and query the device feature feature 
*  related parameters that are not accessible via direct show functions.
* \{
...
DEFINE_GUID(IID_IuEyeImageForcv::Mat, 
0x368f0072, 0x66d3, 0x406f, 0xa7, 0x15, 0xd8, 0x12, 0x8e, 0xad, 0x9b, 0x7d);

interface IuEyeImageForcv::Mat : public IUnknown
{
	/*!	 
	 * \brief	Get the image forcv::Mat list
     * \param 	pListForcv::Mats	Pointer to image forcv::Mat list	
	 * \param	uiSize			Size of image forcv::Mat list
	 * 
     * \return	HRESULT			0 on success, error code otherwise.
	 */
	STDMETHOD (ImageForcv::Mat_GetList)(void* pListForcv::Mats, UINT uiSize) = 0;
     ...
     * \return	HRESULT			0 on success, error code otherwise.
	 */
	STDMETHOD (ImageForcv::Mat_GetNumberOfEntries)(UINT* puiNumber) = 0;

	/*!	 
	 * \brief	Set the ID of the the image forcv::Mat
     * \param 	uiForcv::MatID		ID of the the image forcv::Mat		
	 * 
     * \return	HRESULT			0 on success, error code otherwise.
	 */
	STDMETHOD (ImageForcv::Mat_SetForcv::Mat)(UINT uiForcv::MatID) = 0;
     ...
     * \return	HRESULT			0 on success, error code otherwise.
	 */
	STDMETHOD (ImageForcv::Mat_GetArbitraryAOISupported)(UINT* puiSupported) = 0;

};


// ============================================================================
/*! \defgroup IuEyeColorConverter uEye image forcv::Mat Interface
* 
